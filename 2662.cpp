...
public:
    int minimumCost(vector<int> &start, vector<int> &target, vector<vector<int>> &specialRoads)
    {
        ...
        while (q.size())
        {
            ...
            {
                int i = s[0], j = s[1], k = s[2], l = s[3], cost = s[4];

                int new_total_neg = total_neg - abs(x - i) - abs(y - j) - cost;
                array<int, 3> to_push = {new_total_neg, k, l};
                if (!dp.count({k, l}) || -new_total_neg < dp[{k, l}])
                    q.push(to_push), dp[{k, l}] = -new_total_neg;
            }

            q.push({total_neg - abs(target[0] - x) - abs(target[1] - y), target[0], target[1]});
        }
